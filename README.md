...
*   **🤖 AI 自动翻译 (Powered by Gemini Flash)**：
    ...
    *   *(注：此功能需要配置 API Key，未配置时不影响其他功能使用)*
...
*   **(可选) Google Gemini API Key**: 仅在使用 AI 翻译功能时需要。如果不配置，您可以正常使用除翻译外的所有功能。
...
2.  **制作歌词**：
    ...
    *   **翻译**：在编辑器中，选择源语言和目标语言，点击 "Create Bilingual" 生成双语歌词（需配置 API Key）。
